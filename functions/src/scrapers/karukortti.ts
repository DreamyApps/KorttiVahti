...
export class KaruKorttiScraper extends BaseScraper {
  readonly storeId = 'karukortti';
  readonly storeName = 'Karukortti';
  readonly baseUrl = 'https://www.karukortti.fi';
    ...
    try {
      ...
      for (const url of urls) {
        try {
          ...
          // WooCommerce-style product listings
          $('.product, .product-card, li.type-product').each((_, el) => {
            try {
              const $el = $(el);
              const name = $el.find('.woocommerce-loop-product__title, h2, h3, .product-title').first().text().trim();
              const priceText = $el.find('.price .amount, .price ins .amount, .price > .amount').first().text().trim()
                || $el.find('.price').first().text().trim();
              const link = $el.find('a.woocommerce-LoopProduct-link, a').first().attr('href') ?? '';
              const img = $el.find('img').first().attr('src') ?? $el.find('img').first().attr('data-src') ?? '';
              const outOfStock = $el.hasClass('outofstock') || $el.find('.out-of-stock, .sold-out').length > 0;
